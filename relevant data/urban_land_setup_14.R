...
# Load flood map (global coverage)
floodMap<- raster(file.path('..', 'data', 'flood', 'floodMapGL_rp100y.tif'))
#unsuitability[is.na(unsuitability)] <- 0
...
for (iso in lst_countries) {
  ...
  if(nrow(country)>0 & !dir.exists(path_ctry)) {
    ...
    country_2 <- country %>%
      sf::st_transform(crs = sf::st_crs(floodMap))
      
    mask_2_ctry <- country_2 %>%
      fasterize::fasterize(floodMap) %>%
      raster::crop(country_2)
    ...
    # Crop the flood map to the country
    floodMap_ctry<- floodMap %>%
      raster::crop(mask_2_ctry) %>%
      raster::mask(mask_2_ctry)
    ...
    # Interpolate suitability to the floodmap
    suit_2_ctry <- floodMap_ctry %>%
      raster::projectRaster(suit_ctry) %>%
      raster::resample(suit_ctry, method='bilinear')
    
    suit_2_ctry[suit_2_ctry < 0] <- NA
    
    #suitbility_3<-suit_2_ctry %>%
    #raster::mask(suit_ctry) 
    ...
    floodMap_ctry %>%
      raster::projectRaster(crs = 'EPSG:6933', method = 'ngb') %>%
      raster::writeRaster(filename = file.path('..', 'results', iso, 'floodMap_ctry.tif'),
                          overwrite = T)
    ...
    suit_2_ctry %>%
      raster::projectRaster(crs = 'EPSG:6933') %>%
      raster::writeRaster(filename = file.path('..', 'results', iso, 'suitability_flood.tif'),
                          overwrite = T)
  }
}
